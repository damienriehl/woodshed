...
export default {async fetch(request:Request,env:WorkerBindings):Promise<Response>{
  try{
    ...
    if(url.pathname==="/api/logout"&&request.method==="POST"){mutationGuard(request,env);const responseHeaders=new Headers(headers),sessions=new Set<string>();const bearer=bearerToken(request);if(bearer)sessions.add(bearer);for(const [name,value] of Object.entries(cookies(request)))if(name==="woodshed_session"||/^woodshed_session_[0-9a-f]{16}$/.test(name)){sessions.add(value);responseHeaders.append("set-cookie",`${name}=; Max-Age=0; Path=/; HttpOnly${env.APP_ORIGIN.startsWith("https://")?"; Secure":""}; SameSite=Lax`);}for(const value of sessions)await choice.revokeSession(value);return new Response(null,{status:204,headers:responseHeaders});}
