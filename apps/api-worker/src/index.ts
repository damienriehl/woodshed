...
export default {async fetch(request:Request,env:WorkerBindings):Promise<Response>{
  try{
    ...
    if(url.pathname==="/api/logout"&&request.method==="POST"){mutationGuard(request,env);const responseHeaders=new Headers(headers);for(const name of Object.keys(cookies(request)))if(name==="woodshed_session"||/^woodshed_session_[0-9a-f]{16}$/.test(name))responseHeaders.append("set-cookie",`${name}=; Max-Age=0; Path=/; HttpOnly${env.APP_ORIGIN.startsWith("https://")?"; Secure":""}; SameSite=Lax`);return new Response(null,{status:204,headers:responseHeaders});}
