...
test("legacy session cookie keeps an authorized unlisted event discoverable",async()=>{const service=new ChoiceService(":memory:");service.migrate();service.seedDemo();const invite=service.issueInvite("event_unlisted","participant"),session=service.exchangeInvite(invite.capability),app=createApi(service,{origin:"https://woodshed.example"}),active=await app.request("/api/session/events",{headers:{cookie:`woodshed_session=${session.id}`}});assert.deepEqual(await active.json(),{events:[{id:"event_unlisted",name:"Band Workshop",state:"published",visibility:"unlisted",participationPolicy:"invite"}]});assert.equal((await app.request("/api/events/event_unlisted/context",{headers:{cookie:`woodshed_session=${session.id}`}})).status,200);service.close();});
...
test("stale recovery cookie does not block a normal open-event join",async()=>{const origin="http://127.0.0.1:5173",service=new ChoiceService(":memory:");service.migrate();service.seedDemo({publicParticipationPolicy:"open"});const app=createApi(service,{origin}),headers={origin,"x-csrf-token":"same-origin","content-type":"application/json"},first=await app.request("/api/events/event_public/join-open",{method:"POST",headers,body:JSON.stringify({operationId:"join_stale_first"})}),recoveryCookie=(first.headers.getSetCookie?.()??[]).find(value=>value.startsWith("woodshed_recovery_"))?.split(";")[0]??"";assert.ok(recoveryCookie);service.database.prepare("UPDATE participation_recovery SET expires_at='2000-01-01T00:00:00Z'").run();const joined=await app.request("/api/events/event_public/join-open",{method:"POST",headers:{...headers,cookie:recoveryCookie},body:JSON.stringify({operationId:"join_after_stale"})});assert.equal(joined.status,200);assert.equal(Number((service.database.prepare("SELECT count(*) count FROM guest_participations WHERE event_id='event_public'").get() as {count:number}).count),2);service.close();});

test("removed participation disappears from active session events",async()=>{const origin="http://127.0.0.1:5173",service=new ChoiceService(":memory:");service.migrate();service.seedDemo({publicParticipationPolicy:"open"});const app=createApi(service,{origin}),response=await app.request("/api/events/event_public/join-open",{method:"POST",headers:{origin,"x-csrf-token":"same-origin","content-type":"application/json"},body:JSON.stringify({operationId:"join_removed"})}),cookie=(response.headers.getSetCookie?.()??[]).find(value=>value.startsWith("woodshed_session_"))?.split(";")[0]??"",participation=service.database.prepare("SELECT id FROM guest_participations WHERE event_id='event_public'").get() as {id:string};service.removeParticipant(participation.id);const active=await app.request("/api/session/events",{headers:{cookie}});assert.deepEqual(await active.json(),{events:[]});assert.equal((await app.request("/api/events/event_public/ballot",{headers:{cookie}})).status,401);service.close();});
...
test("logout revokes copied recovery credentials and their sessions",async()=>{const origin="http://127.0.0.1:5173",service=new ChoiceService(":memory:");service.migrate();service.seedDemo({publicParticipationPolicy:"open"});const app=createApi(service,{origin}),headers={origin,"x-csrf-token":"same-origin","content-type":"application/json"},joined=await app.request("/api/events/event_public/join-open",{method:"POST",headers,body:JSON.stringify({operationId:"logout_recovery"})}),setCookies=joined.headers.getSetCookie?.()??[],sessionCookie=setCookies.find(value=>value.startsWith("woodshed_session_"))?.split(";")[0]??"",recoveryCookie=setCookies.find(value=>value.startsWith("woodshed_recovery_"))?.split(";")[0]??"";assert.ok(sessionCookie&&recoveryCookie);const copiedRecovery=recoveryCookie;const logout=await app.request("/api/logout",{method:"POST",headers:{...headers,cookie:`${sessionCookie}; ${recoveryCookie}`},body:"{}"});assert.equal(logout.status,204);assert.equal((await app.request("/api/events/event_public/ballot",{headers:{cookie:sessionCookie}})).status,401);assert.equal((await app.request("/api/events/event_public/recover",{method:"POST",headers:{...headers,cookie:copiedRecovery},body:"{}"})).status,401);service.close();});
...
async function waitForServerExit(child: ChildProcess) {
  if (child.exitCode !== null || child.signalCode !== null) return;
  await new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("server did not exit")), 3_000);
...
test("Node server entrypoint serves synthetic demo through real HTTP and SQLite", { timeout: 15_000 }, async t => {
  ...
  assert.deepEqual(body.events.map((event: { id: string }) => event.id), ["event_public"]);
