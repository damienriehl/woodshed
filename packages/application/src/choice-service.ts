...
export class ChoiceService {
  ...
  propose(sessionId:string,title:string,operationId:string){
    const s=this.session(sessionId), requestHash=hash(JSON.stringify({title})),old=this.database.prepare("SELECT request_hash,result_json FROM proposal_receipts WHERE participation_id=? AND operation_id=?").get(s.participationId,operationId) as {request_hash:string;result_json:string}|undefined;if(old){if(old.request_hash!==requestHash)throw new ChoiceError("replay-mismatch");return JSON.parse(old.result_json);}
    const count=Number((this.database.prepare("SELECT count(*) n FROM choice_proposals WHERE participation_id=?").get(s.participationId) as {n:number}).n);if(count>=3)throw new ChoiceError("quota-exceeded");
    const config=this.database.prepare("SELECT proposal_policy FROM event_choice_config WHERE event_id=?").get(s.eventId) as {proposal_policy:string};const result={id:`proposal_${hash(`${s.participationId}:${operationId}`).slice(0,24)}`,title,state:config.proposal_policy==="immediate"?"eligible":"submitted"};
    this.database.prepare("INSERT INTO choice_proposals(id,event_id,participation_id,title,state,created_at) VALUES (?,?,?,?,?,?)").run(result.id,s.eventId,s.participationId,title,result.state,this.now().toISOString());this.database.prepare("INSERT INTO proposal_receipts(participation_id,operation_id,request_hash,result_json) VALUES (?,?,?,?)").run(s.participationId,operationId,requestHash,JSON.stringify(result));return result;
  }
