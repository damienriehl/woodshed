...
export class ChoiceService {
  ...
  openPublicSession(eventId: string, operationId: string, replaySessionId?: string): Session|IssuedSession {
    ...
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const operationHash=hash(operationId),receipt=this.database.prepare("SELECT participation_id,created_at FROM open_join_receipts WHERE event_id=? AND operation_id=?").get(eventId,operationHash) as {participation_id:string;created_at:string}|undefined;
      if(receipt){
