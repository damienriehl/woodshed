...
import { parseCommandEnvelope, type CommandEnvelope } from "../../contracts/src/index.ts";
import { replaceBallot as replaceBallotDomain } from "../../domain/src/ballot.ts";

type FailurePoint = "after-state" | "after-audit";
type SyntheticFirstLoopIds = {
  community: string;
...
export class SqliteKernel {
  ...
  seedSyntheticFirstLoop(ids: SyntheticFirstLoopIds) {
  ...
  replaceBallot(rawEnvelope: unknown, rankings: readonly string[], now = new Date(), failurePoint?: FailurePoint) {
    const envelope = parseCommandEnvelope(rawEnvelope);
    const scopedEventId = envelope.eventId;
    if (!scopedEventId) throw new Error("ballot replacement requires event scope");
    if (envelope.capability !== "ballot:replace") throw new Error("invalid capability for ballot replacement");
    if (envelope.aggregateType !== "ballot") throw new Error("invalid aggregate type for ballot replacement");
    if (envelope.aggregateId !== `${envelope.actorId}:${scopedEventId}`) throw new Error("invalid ballot aggregate identity");
    if (now.getTime() > Date.parse(envelope.expiresAt)) throw new Error("command expired");
    if (now.getTime() < Date.parse(envelope.issuedAt)) throw new Error("command not yet valid");
    ...
    if (receipt) {
      if (receipt.payload_hash !== payloadHash) throw new Error("operation ID payload mismatch");
      return JSON.parse(receipt.result_json) as { revision: number; rankings: string[] };
    }
    ...
    try {
      ...
      if (!scope) throw new Error("community or event scope denied");
      ...
      if (!current || current.current_revision !== envelope.expectedRevision) throw new Error(`stale revision: expected ${envelope.expectedRevision}`);
      ...
      const outcome = replaceBallotDomain(current.current_revision === 0 ? undefined : { revision: current.current_revision, rankings: [] }, { rankings, eligibleSongIds: eligible }).current;
      ...
      if (cas.changes !== 1) throw new Error("stale revision during mutation");
      ...
      const result = { revision: outcome.revision, rankings: [...outcome.rankings] };
